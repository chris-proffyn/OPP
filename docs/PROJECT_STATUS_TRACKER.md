...
**Last Updated:** Bulk test user creation (§15 security and logging); documentation pass
...
- **Checkout update (route display):** Per `docs/OPP_CHECKOUT_UPDATE_DOMAIN.md` and `docs/OPP_CHECKOUT_UPDATE_IMPLEMENTATION_CHECKLIST.md`. Game Engine shows checkout label, Start/Remaining, recommended combination and player variation (by total) during checkout steps; get-by-total APIs in @opp/data; edge cases (remaining 0, &gt;170, &lt;2; no combination; null darts as "—") handled. See checklist §1–§5.
- **Routine page (session / step split):** Per `docs/OPP_ROUTINE_PAGE_DOMAIN.md` and `docs/OPP_ROUTINE_PAGE_IMPLEMENTATION_CHECKLIST.md`. **Routine page flow:** Play → choose session → session view (player, TR, session name, session score, current routine; Start/Resume, “Continue to step”) → step view (routine name, instructions, target, visit breakdown, SegmentGrid, voice/manual, Correct visit, Submit) → on step complete return to session view; on session complete show final score and “Back to Play”. State in `SessionGameContext`; layout `PlaySessionLayout` wraps session + step routes. Data: same `@opp/data` (revertLastVisit, listDartScoresForStep, etc.). See checklist §1–§10.
- **Bulk test user creation:** Per `docs/OPP_BULK_TEST_USER_CREATION_DOMAIN.md` and `docs/OPP_BULK_TEST_USER_CREATION_IMPLEMENTATION_CHECKLIST.md`. Admin-only bulk create/delete of test users via Supabase Edge Functions (`bulk-create-test-users`, `bulk-delete-test-users`). Data model: `players` test-user columns, `test_user_jobs`, `test_user_seq`; RPC `allocate_test_user_n`. Create: JWT + admin check, rate limit (200/request, 500/hour per admin), production safeguard (`confirm_test_users`), job tracking, randomised profile bootstrap (idempotent upsert). Delete: by job_id, n_range, or all test users. Admin UI at `/admin/test-users` (create form, results, bulk delete). **§15 Security and logging:** Service role key only in Edge Function env (never in client); server-side JSON logging (job_id, user_id, email, n; per-user errors; no password); idempotent bootstrap. See checklist §15 and function comments.
